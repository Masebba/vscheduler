...
const AdminFaculty = () => {
  ...
  const startEdit = asg => {
    setEditingId(asg.id);
    setCourseNameFilter(asg.course);
    setCodeSelections(
      courses.filter(c => c.name === asg.course).map(c => c.id)
    );
  ...
  const saveEdit = async id => {
    for (let cid of codeSelections) {
      ...
      setAssignments(a =>
        a.map(x =>
          x.id === id
            ? { ...x, course: course.name, courseCode: course.code }
            : x
        )
      );
    }
    ...
    <div className="flex h-screen bg-stone-100">
      ...
      <div className="flex-1 ml-64 p-4">
        ...
        <div className="mb-4 flex flex-wrap gap-2 items-start">
          <select value={facultyFilter} onChange={e => setFacultyFilter(e.target.value)} className="border p-1 text-sm">
          ...
          <select value={courseNameFilter} onChange={e => setCourseNameFilter(e.target.value)} disabled={!facultyFilter} className="border p-1 text-sm">
            <option value="">Select Module</option>
            {courseNames.map(n => <option key={n}>{n}</option>)}
          </select>
          {courseNameFilter && <button onClick={() => setShowCodeDialog(true)} className="bg-gray-300 px-2 py-1 text-sm rounded">Select Codes</button>}
          <button onClick={editingId ? () => saveEdit(editingId) : handleAssign} className="bg-sky-700 text-white px-2 py-1 text-sm rounded">
            {editingId ? 'Save Changes' : 'Assign Modules'}
          </button>
        </div>
        ...
        {showCodeDialog && (
          <div className="fixed inset-0 bg-black bg-opacity-30 flex justify-center items-center">
            <div className="bg-white p-4 rounded shadow-lg max-h-96 overflow-auto w-80">
              ...
              <div className="mt-4 text-right">
                <button onClick={() => {
                  ...
                  setCourseNameFilter("");
                  setCodeSelections([]);
                }} className="mr-2 px-3 py-1 border rounded">Cancel</button>
                <button onClick={() => editingId ? saveEdit(editingId) : handleAssign()} className="bg-blue-600 text-white px-3 py-1 rounded">OK</button>
              </div>
            </div>
          </div>
        )}
            ...
            {assignments.map(x => (
              <tr key={x.id} className="even:bg-gray-100 text-center">
                ...
                <td className="border p-1 space-x-1">
                  ...
                  <button onClick={() => handleDelete(x.id)} className="bg-red-500 text-white px-2 py-1 text-xs rounded">Del</button>
                </td>
              </tr>
            ))}
          </tbody>
